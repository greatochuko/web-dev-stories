...
export default function PostGrid({
  ...
  return (
    <section className={`pb-10 ${grayBg ? "bg-zinc-100" : ""} text-zinc-700`}>
      {title && (
        <h1 className="mb-4 font-semibold text-2xl max-w-7xl  mx-auto px-4 pt-4 flex justify-between">
          {title}
          {url && (
            <Link
              to={url}
      ...
      <div className="grid gap-8 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4  max-w-7xl mx-auto p-4">
        {loading ? (
          <>
            <PostWireFrame />
            <PostWireFrame />
